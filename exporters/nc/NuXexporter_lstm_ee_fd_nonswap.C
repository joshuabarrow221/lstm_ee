...
const std::string DATA =
    ...
    " prod_sumdecaf_R19-11-18-prod5reco.f_fd_genie_N1810j0211a_nonswap_fhc_nova_v08_full_v1_nus2020";
...
const Cut cut = kNus20FDCuts && SanityCut;

const std::vector<std::pair<std::string, Var>> TRUTH_VAR_DEFS({
    { "mode",     SIMPLEVAR(mc.nu[0].mode) },
    { "trueE",    SIMPLEVAR(mc.nu[0].E) },
    { "trueLepE", SIMPLEVAR(mc.nu[0].prim[0].p.E) }, // Primary daughters, lepton comes first in prim vector.
    { "trueHadE",
      Var(
...
void NuXexporter_lstm_ee_fd_nonswap()
{
    CSVMaker maker(DATA, "dataset_NuX_lstm_ee_fd_nonswap.csv");
